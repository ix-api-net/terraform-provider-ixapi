...
func (c *Client) NetworkServiceConfigPrefixesList(
	ctx context.Context,
	nscID string,
	mode string,
) ([]*BGPPrefix, error) {
	params := ""
	if mode != "" {
		params = fmt.Sprintf("?mode=%s", mode)
	}

	url := c.resourceURL("/api/v3/decix-vrf-v1/network-service-configs/{id}/prefixes"+params, nscID)
	hreq, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	if err != nil {
		return nil, err
	}
