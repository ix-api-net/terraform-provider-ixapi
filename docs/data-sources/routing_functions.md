---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "ixapi_routing_functions Data Source - terraform-provider-ixapi"
subcategory: ""
description: |-
  Use the ixapi_routing_functions data source to find available routing functions
---

# ixapi_routing_functions (Data Source)

Use the `ixapi_routing_functions` data source to find available routing functions



<!-- schema generated by tfplugindocs -->
## Schema

### Optional

- `consuming_account` (String) Filter by account consuming the routing function
- `external_ref` (String) Filter by external reference
- `managing_account` (String) Filter by account managing the routing function
- `routing_functions` (Block List) (see [below for nested schema](#nestedblock--routing_functions))
- `state` (String) Filter by state

### Read-Only

- `id` (String) The ID of this resource.

<a id="nestedblock--routing_functions"></a>
### Nested Schema for `routing_functions`

Optional:

- `asn` (Number) Any routing function instance needs to be assigned a 2-byte or 4-byte ASN of the customer's choice. There is no restriction on private or public ASNs.
- `billing_account` (String) An account requires billing_information to be used as a `billing_account`. *(Sensitive Property)*
- `capacity` (Number) The desired upper bound of the capacity for the routing function.
- `consuming_account` (String) The `id` of the account consuming a service.  Used to be `owning_customer`. *(Sensitive Property)*
- `contract_ref` (String) A reference to a contract. If no specific contract is used, a default MAY be chosen by the implementer. *(Sensitive Property)*
- `external_ref` (String) Reference field, free to use for the API user. *(Sensitive Property)*
- `id` (String) The *primary identifier* of the `Routing Function`.
- `managing_account` (String) The `id` of the account responsible for managing the service via the API. A manager can read and update the state of entities. *(Sensitive Property)*
- `product_offering` (String) The product offering to be used for the routing function.
- `purchase_order` (String) Purchase Order ID which will be displayed on the invoice. *(Sensitive Property)*
- `state` (String) The state of the object. *(Sensitive Property)*
- `status` (Block List) (see [below for nested schema](#nestedblock--routing_functions--status))

<a id="nestedblock--routing_functions--status"></a>
### Nested Schema for `routing_functions.status`

Optional:

- `message` (String) A human readable message, describing the problem and may contain hints for resolution.
- `severity` (Number) We are using syslog severity levels: 0 = Emergency, 1 = Alert, 2 = Critical, 3 = Error, 4 = Warning, 5 = Notice, 6 = Informational, 7 = Debug.
- `tag` (String) A machine readable message identifier.
- `timestamp` (String) The time and date when the event occured.


