...
page_title: "ixapi_network_service_mp2mp_vc Data Source - terraform-provider-ixapi"
subcategory: ""
description: |-
...
- `mac_acl_protection` (Boolean) When enabled, only MAC addresses in the referenced in the network service config's `macs` property are allowed to send and receive traffic on this network service.
